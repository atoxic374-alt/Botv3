...
function createClient(proxyUrl) {
  ...
  if (proxyUrl && typeof proxyUrl === 'string' && proxyUrl.trim()) {
    ...
    // https-proxy-agent v9 does NOT propagate rejectUnauthorized through the TLS tunnel
    // to the final target (discord.com). BrightData uses its own CA cert for HTTPS
    // interception — the only reliable fix is the NODE_TLS environment flag.
  ...
  const http = axios.create({
    // Discord JSON calls must fail fast enough for the UI to recover.
    timeout: 15_000,
    validateStatus: () => true,
    httpAgent,
    httpsAgent,
...
async function fetchBuildNumber(client) {
  try {
    const page = await _warmupRequest(client, {
      method: 'GET',
    ...
    // Build scraping is best-effort only. Never wait on a long tail of stale
    // chunks when the static build number is already sufficient for the API.
    for (const src of scripts.slice(-3).reverse()) {
      try {
        const r = await _warmupRequest(client, {
          method: 'GET',
          url: 'https://discord.com' + src,
          headers: {
            'Accept':          '*/*',
            'Accept-Language': 'en-US,en;q=0.9',
...
function _isNetworkError(error) {
  ...
  return /^(ECONN|ETIMEDOUT|EAI_AGAIN|EHOST|ENET|EPROTO|ERR_TLS)/.test(code)
    || /socket|tls|network|secure connection|timeout|connect/i.test(message);
}

function _networkErrorMessage(error) {
  if (_isNetworkError(error)) {
    return 'تعذر الاتصال بـ Discord مؤقتاً (TLS/الشبكة). أعد المحاولة بعد قليل.';
  }
  return String(error?.message || error || 'فشل اتصال Discord').slice(0, 220);
}

async function _warmupRequest(client, config, attempts = WARMUP_ATTEMPTS) {
  let lastError;
  for (let attempt = 0; attempt < Math.max(1, attempts); attempt++) {
    try {
      return await client.http({ ...config, timeout: config.timeout || WARMUP_TIMEOUT_MS });
    } catch (error) {
      lastError = error;
      if (attempt + 1 < attempts) await _sleep(250);
    }
  }
  throw lastError;
}
...
function _bodySnippet(data) {
  if (data == null) return '';
  if (typeof data === 'string') return data.replace(/\s+/g, ' ').slice(0, 500);
  try { return JSON.stringify(data).slice(0, 500); } catch (_) { return String(data).slice(0, 500); }
}

function _brightDataProxyError(r) {
  const headers = r?.headers || {};
  const brdCode = _firstHeader(headers, 'x-brd-err-code') || '';
  const brdErr = _firstHeader(headers, 'x-brd-error') || '';
  const brdMsg = _firstHeader(headers, 'x-brd-err-msg') || _firstHeader(headers, 'proxy-status') || '';
  const body = _bodySnippet(r?.data);
  const combined = [brdCode, brdErr, brdMsg, body].filter(Boolean).join(' — ');
  if (!/brightdata|brd\.superproxy|x-brd|client_\d+|not an allowed target|target .*whitelist|ip_forbidden|policy_20140|bad_endpoint|immediate residential|no KYC|robots\.txt/i.test(combined)) {
    return null;
  }
  if (/policy_20140|bad_endpoint|immediate residential|no KYC|robots\.txt|Requested site is not available/i.test(combined)) {
    return {
      code: 'PROXY_POLICY_BLOCKED',
      message: 'Bright Data Residential Immediate Access يمنع هذا الهدف (policy_20140/bad_endpoint) — أكمل KYC أو استخدم Zone/منتج يسمح بهذا الهدف',
      detail: combined,
    };
  }
  if (/client_10250|not an allowed target|target .*whitelist/i.test(combined)) {
    const host = combined.match(/target\s+([a-z0-9.-]+)\s+but got blocked/i)?.[1]
      || combined.match(/\bdiscord(?:app)?\.com\b/i)?.[0]
      || combined.match(/\b[a-z0-9.-]+\.[a-z]{2,}\b/i)?.[0]
      || 'target host';
    return {
      code: 'PROXY_TARGET_BLOCKED',
      message: `Bright Data target whitelist يمنع ${host} — أضف discord.com وdiscordapp.com إلى Target whitelist أو احذف whitelist للسماح بالتشغيل`,
      detail: combined,
    };
  }
  if (/client_10030|ip_forbidden|not whitelisted|Allowed IPs/i.test(combined)) {
    const ip = combined.match(/\b\d{1,3}(?:\.\d{1,3}){3}\b/)?.[0] || '';
    return {
      code: 'PROXY_IP_FORBIDDEN',
      message: `Bright Data IP allowlist يمنع الخادم${ip ? ` (${ip})` : ''}`,
      detail: combined,
    };
  }
  return {
    code: 'PROXY_ERROR',
    message: 'Bright Data proxy رفض الطلب',
    detail: combined,
  };
}
...
function _discordErrorDetail(data) {
...
function _isPhoneVerificationMessage(msg) {
  return /phone|verify phone|phone verification|رقم|هاتف/i.test(String(msg || ''));
}
...
async function _req({
  ...
  let r;
  try {
    r = await client.http({ method, url, data: body, headers });
  } catch (error) {
    const wrapped = new DiscordError(_networkErrorMessage(error), {
      code: _isNetworkError(error) ? 'NETWORK_ERROR' : (error?.code || 'REQUEST_FAILED'),
    });
    wrapped.cause = error;
    throw wrapped;
  }
  ...
  const proxyErr = r.status >= 400 ? _brightDataProxyError(r) : null;
  if (proxyErr) {
    throw new DiscordError(`${proxyErr.message}${proxyErr.detail ? ' | ' + proxyErr.detail.slice(0, 240) : ''}`, {
      status: r.status,
      data: r.data,
      code: proxyErr.code,
    });
  ...
  if (r.status === 429) {
    ...
    // Only a clear block signal is treated as a Cloudflare block. Missing
    // optional headers or code=0 alone are ambiguous and must remain a normal
    // 429 so the server-provided retry window is respected.
    const _cfMsg = String(r.data?.message || '').toLowerCase();
    const _cfBody = typeof r.data === 'string' ? r.data.toLowerCase() : '';
    const _isCf  = /blocked|error\s*1015|cloudflare|temporarily restricted|attention required/i.test(`${_cfMsg} ${_cfBody}`);
    if (_isCf) {
...
// ─────────────────────────────────────────────────────────────────────────────
async function warmUpClient(client, { refreshBuild = process.env.BOTV3_LIVE_BUILD === '1' } = {}) {
  if (client.warmedUp) return client.warmupResult || { ok: true, cached: true };

  // Live build scraping is optional. The static build is adequate for normal
  // API calls and avoids several serial asset downloads on every fresh session.
  if (refreshBuild) {
  ...
  try {
    ...
    warmupError = { code: error?.code || 'WARMUP_NETWORK_ERROR', message: _networkErrorMessage(error) };
  }

  if (loginLoaded) {
    await _humanDelay(250, 650);
    try {
      const er = await _warmupRequest(client, {
        method: 'GET',
        url: `${API}/experiments`,
        headers: {
          'X-Super-Properties':  client.superPropsB64,
  ...
  client.warmupResult = {
    ok: loginLoaded,
    loginLoaded,
    fingerprintLoaded: !!client.fingerprint,
    error: warmupError,
  };
  return client.warmupResult;
}
...
async function login({ email, password, totpSecret, netOpts = {} }) {
  ...
  const warmup = await warmUpClient(netOpts.client);
  // Keep a short pacing pause, but do not make a failed/partial warm-up
  // block account verification for several seconds.
  await _humanDelay(warmup?.ok ? 500 : 150, warmup?.ok ? 1100 : 350);
...
async function listTeamApplications({ token, teamId, netOpts = {} }) {
  ...
  if (r.status >= 400) {
    ...
    throw new DiscordError('listTeamApplications failed: ' + (r.data?.message || r.status), {
      status: r.status,
      data: r.data,
      code: r.status === 429 ? 'RATE_LIMITED' : (r.data?.code || 'TEAM_APPS_FAILED'),
      retryAfter: rateLimit.retryAfter || rateLimit.resetAfter || 0,
      rateLimit,
    });
  }
  if (!Array.isArray(r.data)) {
    throw new DiscordError('listTeamApplications returned an invalid response', {
      status: r.status, data: r.data, code: 'INVALID_RESPONSE',
    });
  }
  return r.data;
}
...
async function getCurrentUser({ token, netOpts = {} }) {
  ...
  if (r.status >= 400) {
    const rateLimit = rateLimitInfoFromResponse(r);
    throw new DiscordError('getCurrentUser failed: ' + (r.data?.message || r.status), {
      status: r.status,
      data: r.data,
      code: r.status === 429 ? 'RATE_LIMITED' : (r.data?.code || 'CURRENT_USER_FAILED'),
      retryAfter: rateLimit.retryAfter || rateLimit.resetAfter || 0,
      rateLimit,
    });
  }
  if (!r.data?.id) {
    throw new DiscordError('getCurrentUser returned no user id', {
      status: r.status, data: r.data, code: 'INVALID_RESPONSE',
    });
  }
  return r.data;
...
async function createTeam({ token, name, netOpts = {} }) {
  ...
  if (r.status >= 400) {
    ...
    throw new DiscordError(
      ...
      (r.data?.code ? ` (code ${r.data.code})` : ''),
      {
        status: r.status,
        data: r.data,
        code: r.status === 429 ? 'RATE_LIMITED' : (r.data?.code || 'CREATE_TEAM_FAILED'),
        retryAfter: rateLimit.retryAfter || rateLimit.resetAfter || 0,
        rateLimit,
      },
    );
  ...
  if (!r.data?.id) {
    throw new DiscordError('createTeam returned no team id', {
      status: r.status, data: r.data, code: 'INVALID_RESPONSE',
    });
  }
...
async function listApplications({ token, netOpts = {} }) {
  ...
  if (!Array.isArray(r.data)) {
    throw new DiscordError('listApplications returned an invalid response', {
      status: r.status, data: r.data, code: 'INVALID_RESPONSE',
    });
  }
  return r.data;
...
async function createApplication({ token, name, teamId = null, netOpts = {} }) {
  ...
  if (r.status >= 400) {
    ...
    if (r.status === 403) {
      const detail = _discordErrorDetail(r.data);
      if (_isPhoneVerificationMessage(msg)) {
        throw new DiscordError('الحساب يحتاج تحقق هاتفي قبل إنشاء التطبيقات' + (detail ? ` (${detail})` : ''), {
          status: r.status, data: r.data, code: 'PHONE_VERIFY',
        });
      }
      throw new DiscordError(`Discord رفض إنشاء التطبيق بصلاحية 403${detail ? ` (${detail})` : `: ${msg}`}`, {
        status: r.status, data: r.data, code: r.data?.code || 'FORBIDDEN',
      });
    }
    if (r.status === 429) {
      const rateLimit = rateLimitInfoFromResponse(r);
    ...
    if (r.status === 400 && _isPhoneVerificationMessage(msg)) {
    ...
    const rateLimit = rateLimitInfoFromResponse(r);
    throw new DiscordError(`createApplication فشل (${r.status}): ${msg}${rawHint}`, {
      status: r.status,
      data: r.data,
      code: r.status === 429 ? 'RATE_LIMITED' : (r.data?.code || 'CREATE_APPLICATION_FAILED'),
      retryAfter: rateLimit.retryAfter || rateLimit.resetAfter || 0,
      rateLimit,
    });
  }
  if (!r.data?.id) {
    throw new DiscordError('createApplication returned no application id', {
      status: r.status, data: r.data, code: 'INVALID_RESPONSE',
    });
  }
...
async function ensureBot({ token, appId, netOpts = {} }) {
  ...
  // A 400 existing-bot response is not proof by itself. Re-read the
  // application and require a concrete bot user before continuing.
  if (r.status === 400 && (r.data?.code === 30007 || r.data?.code === 30013)) {
    const verified = await getApplication({ token, appId, netOpts });
    if (verified?.bot?.id) return verified;
    throw new DiscordError('ensureBot returned an existing-bot response but no bot user was verified', {
      status: r.status, data: r.data, code: 'BOT_NOT_VERIFIED',
    });
  }
  if (r.status >= 400) {
    const rateLimit = rateLimitInfoFromResponse(r);
  ...
  if (!r.data?.id && !r.data?.bot?.id) {
    const verified = await getApplication({ token, appId, netOpts });
    if (!verified?.bot?.id) {
      throw new DiscordError('ensureBot returned success without a verified bot user', {
        status: r.status, data: r.data, code: 'BOT_NOT_VERIFIED',
      });
    }
    return verified;
...
async function setApplicationIntents({ token, appId, enabled = true, netOpts = {}, app = null }) {
  ...
  if (nextFlags === current) {
    const currentState = normalizeIntentState(app);
    const values = Object.values(currentState.state || {});
    const writableConfirmed = enabled
      ? values.length === 3 && values.every(item => item.enabled)
      : values.length === 3 && values.every(item => !item.limited);
    const fullyConfirmed = enabled
      ? writableConfirmed
      : values.length === 3 && values.every(item => !item.enabled);
    return {
      ...app,
      operation: {
        ...
        writableConfirmed,
        partial: !fullyConfirmed,
        approvedRemain: values.filter(item => item.approved).length,
      },
    };
  }
  const r = await _req({
  ...
  const verified = await getApplication({ token, appId, netOpts });
  const verifiedState = normalizeIntentState(verified);
  const values = Object.values(verifiedState.state || {});
  const writableConfirmed = enabled
    ? values.length === 3 && values.every(item => item.enabled)
    : values.length === 3 && values.every(item => !item.limited);
  const fullyConfirmed = enabled
...
// Read the account's Nitro boost slots from the client-facing Discord route.
async function getPremiumGuildSubscriptionSlots({ token, netOpts = {} }) {
  const r = await _req({
    method: 'GET',
    url: `${API}/users/@me/guilds/premium/subscription-slots`,
    token,
    netOpts,
  });
  if (r.status >= 400) {
    throw new DiscordError('getPremiumGuildSubscriptionSlots failed: ' + (r.data?.message || r.status), {
      status: r.status, data: r.data, code: r.status === 429 ? 'RATE_LIMITED' : 'NITRO_SLOTS_READ_FAILED',
    });
  }
  return Array.isArray(r.data) ? r.data : [];
}

// Read currently applied Nitro boosts for the account.
async function getPremiumGuildSubscriptions({ token, netOpts = {} }) {
  const r = await _req({
    method: 'GET',
    url: `${API}/users/@me/guilds/premium/subscriptions`,
    token,
    netOpts,
  });
  if (r.status >= 400) {
    throw new DiscordError('getPremiumGuildSubscriptions failed: ' + (r.data?.message || r.status), {
      status: r.status, data: r.data, code: r.status === 429 ? 'RATE_LIMITED' : 'NITRO_SUBSCRIPTIONS_READ_FAILED',
    });
  }
  return Array.isArray(r.data) ? r.data : [];
}

// Read Discord's account-level cooldown metadata. Do not infer a value locally.
async function getPremiumGuildSubscriptionCooldown({ token, netOpts = {} }) {
  const r = await _req({
    method: 'GET',
    url: `${API}/users/@me/guilds/premium/subscriptions/cooldown`,
    token,
    netOpts,
  });
  if (r.status >= 400) {
    throw new DiscordError('getPremiumGuildSubscriptionCooldown failed: ' + (r.data?.message || r.status), {
      status: r.status, data: r.data, code: r.status === 429 ? 'RATE_LIMITED' : 'NITRO_COOLDOWN_READ_FAILED',
    });
  }
  return r.data && typeof r.data === 'object' ? r.data : {};
}

// Apply one or more existing Nitro boost slots to an existing guild.
// This does not create a guild, subscription, account, or boost.
async function applyPremiumGuildSubscriptions({ token, guildId, slotIds, netOpts = {} }) {
  const id = String(guildId || '').trim();
  const ids = Array.isArray(slotIds) ? slotIds.map(v => String(v || '').trim()).filter(Boolean) : [];
  if (!/^\d{5,30}$/.test(id)) throw new DiscordError('Invalid guild id', { code: 'INVALID_GUILD_ID' });
  if (!ids.length) throw new DiscordError('No Nitro boost slots selected', { code: 'NO_NITRO_SLOTS' });
  const r = await _req({
    method: 'PUT',
    url: `${API}/guilds/${encodeURIComponent(id)}/premium/subscriptions`,
    token,
    body: { user_premium_guild_subscription_slot_ids: ids },
    netOpts,
  });
  if (r.status >= 400) {
    const rateLimit = rateLimitInfoFromResponse(r);
    throw new DiscordError('applyPremiumGuildSubscriptions failed: ' + (r.data?.message || r.status), {
      status: r.status, data: r.data,
      code: r.status === 429 ? 'RATE_LIMITED' : (r.data?.code || 'NITRO_BOOST_FAILED'),
      retryAfter: rateLimit.retryAfter || rateLimit.resetAfter || 0,
      rateLimit,
    });
  }
...
// Accept a Discord invite with the selected account's user token.
// This joins an existing server; it does not create a server or account.
async function acceptInvite({ token, inviteCode, netOpts = {} }) {
  const code = String(inviteCode || '').trim();
  if (!/^[A-Za-z0-9_-]{2,100}$/.test(code)) {
    throw new DiscordError('Invalid Discord invite code', { code: 'INVALID_INVITE_CODE' });
  }
  const r = await _req({
    method: 'POST',
    url:    `${API}/invites/${encodeURIComponent(code)}`,
    token,
    body:   {},
    netOpts,
  });
  if (r.status >= 400) {
    ...
    const msg = r.status === 429 ? `Rate limited — retry after ${retryAfter || '?'}s`
      ...
      : r.status === 404 ? 'Invite not found or expired'
      : r.data?.message || String(r.status);
    throw new DiscordError('acceptInvite failed: ' + msg, {
      status: r.status,
      data: r.data,
      code: r.status === 429 ? 'RATE_LIMITED' : (r.data?.code || 'INVITE_ACCEPT_FAILED'),
      retryAfter,
      rateLimit,
    });
  }
...
async function checkBotInGuild({ token, clientId, guildId, netOpts = {} }) {
  const r = await _req({
    method: 'GET',
    url:    `${API}/guilds/${guildId}/members/${clientId}`,
    token, netOpts,
  });
  if (r.status === 200) return true;
  if (r.status === 404) return false;
  const rateLimit = rateLimitInfoFromResponse(r);
  throw new DiscordError('Could not verify bot membership: ' + (r.data?.message || r.status), {
    status: r.status,
    data: r.data,
    code: r.status === 429 ? 'RATE_LIMITED' : 'MEMBERSHIP_CHECK_FAILED',
    retryAfter: rateLimit.retryAfter || rateLimit.resetAfter || 0,
    rateLimit,
  });
}

async function addBotToGuild({ token, clientId, guildId, permissions = '8', netOpts = {}, verifyMember = true }) {
  if (verifyMember) {
    const alreadyPresent = await checkBotInGuild({ token, clientId, guildId, netOpts });
    if (alreadyPresent) return { alreadyPresent: true, verified: true };
  }
  ...
  if (r.status >= 400) {
    ...
    const msg = r.status === 429 ? `Rate limited — retry after ${retryAfter || '?'}s`
      : r.status === 401         ? 'Unauthorized — check account token'
      : r.status === 403         ? 'Missing guild permissions or Discord rejected the invite request'
  ...
  if (verifyMember && !(await checkBotInGuild({ token, clientId, guildId, netOpts }))) {
    throw new DiscordError('Discord accepted the invite request but the bot is not present in the guild', {
      status: 200, data: r.data, code: 'INVITE_NOT_CONFIRMED',
    });
  }
  return { ...(r.data && typeof r.data === 'object' ? r.data : {}), verified: verifyMember };
}
...
async function accountHealthProbe({ token, netOpts = {}, skipUser = false }) {
  const client = netOpts.client || createClient();
  const checks = [];
  async function probe(name, url) {
    ...
    const item = {
      ...
      ready: r.status >= 200 && r.status < 300 && !rateLimit.exhausted,
  ...
  if (!skipUser) await probe('user', `${API}/users/@me`);
  ...
  const pending = !bad && checks.find(c => c.rateLimit?.exhausted);
  let ok = !bad;
  const ready = !bad && !pending;
  let classification = ready ? 'ok' : (pending ? 'rate_limit_pending' : 'unknown');
  let message = ready
    ? 'Account health looks OK'
    : pending
      ? `الحساب صالح لكن rate limit ما زال مفتوحاً على ${pending.name} — انتظر ${pending.retryAfter || pending.resetAfter || '?'} ثانية`
      : (bad?.message || `HTTP ${bad?.status || 'unknown'}`);
  if (bad) {
    if (bad.status === 429) { classification = bad.global ? 'global_rate_limited' : 'rate_limited'; message = `Rate limited (${bad.scope || 'unknown scope'}), retry after ${bad.retryAfter || '?'}s`; }
      ...
      const detail = _discordErrorDetail({ code: bad.code, message: bad.message });
      classification = _isPhoneVerificationMessage(bad.message) ? 'phone_verification_required' : 'forbidden';
      message = _isPhoneVerificationMessage(bad.message)
        ? `Discord طلب تحقق رقم (${detail || '403'})`
        : `Discord رجع 403 في فحص ${bad.name}${detail ? ` (${detail})` : ''}`;
    }
  }
  return {
    ok,
    ...
    message,
    checks,
    rateLimited: !!pending || !!checks.find(c => c.status === 429),
    retryAfter: pending?.retryAfter || bad?.retryAfter || 0,
    resetAfter: pending?.resetAfter || bad?.resetAfter || 0,
    global: !!(pending?.global || bad?.global),
  };
}
...
async function transferAppToTeam({ token, appId, teamId, mfa = null, netOpts = {} }) {
  ...
  if (r.status >= 400) {
    ...
    throw new DiscordError(
      'transferAppToTeam فشل: ' + (r.data?.message || r.status),
      {
        status: r.status,
        data: r.data,
        code: r.status === 429 ? 'RATE_LIMITED' : (r.data?.code || 'TRANSFER_FAILED'),
        retryAfter: rateLimit.retryAfter || rateLimit.resetAfter || 0,
        rateLimit,
      },
    );
  ...
  const verified = await getApplication({ token, appId, netOpts });
  const verifiedTeamId = verified?.team?.id || verified?.team_id || null;
  if (String(verifiedTeamId || '') !== String(teamId)) {
    throw new DiscordError('Application transfer was not confirmed by Discord', {
      status: r.status, data: verified, code: 'TRANSFER_NOT_CONFIRMED',
    });
  }
  return verified;
}
...
function makeSession() {
  return {
    ...
    teamId:          null,
    teamName:        null,
    createdTeams:    [], // confirmed teams created during this session
    bots:            [],
    log:             [],
    lastError:       null,
    waitUntilTs:     0,
    ...
    pendingCaptcha:  null,
    accountRateLimits: {},
    pausedAccounts:  {}, // { email: unPauseTimestampMs } — internal cooldown timestamps
    pausedAccountMeta: {}, // { email: { classification, reason, retryAfterMs } }
  };
}
...
module.exports = {
  ...
  getPremiumGuildSubscriptionSlots,
  getPremiumGuildSubscriptions,
  getPremiumGuildSubscriptionCooldown,
  applyPremiumGuildSubscriptions,
  acceptInvite,
  addBotToGuild,
  checkBotInGuild,
